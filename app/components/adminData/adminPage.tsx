...
const AdminPage: React.FC<adminProps> = ({ users, orders, products }) => {
  ...
  useEffect(() => {
    setStatistics((prev) => {
      ...
      const salesOrders = orders.reduce(
        (acc: number, items: Order) => {
  ...
  return (
    <Box width="100%">
      <SimpleGrid columns={{ base: 2, md: 3 }} spacing={10} p={{base:"6", md:"10", lg:"20"}}>
        {stats &&
          stats.map((key) => (
            <Flex
              direction="column"
              ...
              borderRadius="xl"
            >
              <Heading size={{base:"sm", md:"md"}} mb={2} fontFamily="monospace">{ statistics[key].textLabel}</Heading>
              <Text fontSize="md">{ statistics[key].textLabel === "Total Sales" ? "$" + statistics[key].digit : statistics[key].digit}</Text>
            </Flex>
          ))}
      </SimpleGrid>
    </Box>
